 ...
 * @file       Coin.cpp
 *
 * @brief      PublicCoin and PrivateCoin classes for the acutecoin library.
 *
...
void PrivateCoin::mintCoin(const CoinDenomination denomination) {
	...
	for(uint32_t attempt = 0; attempt < MAX_COINMINT_ATTEMPTS; attempt++) {
		...
		if (coin.getCommitmentValue().isPrime(acuteCOIN_MINT_PRIME_PARAM) &&
	...
	throw acutecoinException("Unable to mint a new acutecoin (too many attempts)");
}
...
void PrivateCoin::mintCoinFast(const CoinDenomination denomination) {
	...
	for (uint32_t attempt = 0; attempt < MAX_COINMINT_ATTEMPTS; attempt++) {
		...
		// in the appropriate range. If not, we'll throw this coin
		// away and generate a new one.
		if (commitmentValue.isPrime(acuteCOIN_MINT_PRIME_PARAM) &&
