...
* @file       Params.h
*
* @brief      Parameter classes for acutecoin.
*
...
public:
	/** @brief Construct a set of acutecoin parameters from a modulus "N".
	* @param N                A trusted RSA modulus
	* @param securityLevel    A security level expressed in symmetric bits (default 80)
	*
	* Allocates and derives a set of acutecoin parameters from
	...
	**/
	...
	/**
	 * Security parameter.
	 * The statistical acute-knowledgeness of the accumulator proof.
	 */
	uint32_t k_dprime;
...
public:
	...
	**/
	Params(Bignum accumulatorModulus,
	       uint32_t securityLevel = acuteCOIN_DEFAULT_SECURITYLEVEL);
