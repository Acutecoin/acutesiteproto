 ...
 * @file       Commitment.cpp
 *
 * @brief      Commitment and CommitmentProof classes for the acutecoin library.
 *
	...
	ap(aParams),bp(bParams)
{
	...
	//
	// We choose these large values to ensure statistical acute knowledge.
...
const Bignum CommitmentProofOfKnowledge::calculateChallenge(const Bignum& a, const Bignum& b, const Bignum &commitOne, const Bignum &commitTwo) const {
	...
	hasher << std::string(acuteCOIN_COMMITMENT_EQUALITY_PROOF);
	hasher << commitOne;
	hasher << std::string("||");
	hasher << commitTwo;
	hasher << std::string("||");
