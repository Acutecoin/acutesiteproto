...
///
/// \brief      Parameter manipulation routines for the acutecoin cryptographic
///             components.
///
...
/// \brief Fill in a set of acutecoin parameters from a modulus "N".
...
///
/// \throws         acutecoinException if the process fails
///
...
/// be known to any party, or the security of acutecoin is
...
CalculateParams(Params &params, Bignum N, string aux, uint32_t securityLevel)
{
	...
	if (NLen < 1023) {
		throw acutecoinException("Modulus must be at least 1023 bits");
	}

	// Verify that "securityLevel" is  at least 80 bits (minimum).
	if (securityLevel < 80) {
		throw acutecoinException("Security level must be at least 80 bits.");
	}
	...
	*pLen = *qLen = 0;
	...
	if (securityLevel < 80) {
		...
		*pLen = 3072;
		...
		throw acutecoinException("Security level not supported.");
	}

	if (*pLen > maxPLen) {
		throw acutecoinException("Modulus size is too small for this security level.");
	}
}
...
deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen)
{
	...
	if ((uint32_t)(result.modulus.bitSize()) < pLen ||          // modulus is pLen bits long
		...
		throw acutecoinException("Group parameters are not valid");
	}
...
deriveIntegerGroupFromOrder(Bignum &groupOrder)
{
	...
	throw acutecoinException("Too many attempts to generate Schnorr group.");
}
                              ...
                              uint256 *resultPseed, uint256 *resultQseed)
{
	...
	if (qLen > (sizeof(seed)) * 8) {
		...
		// throw acutecoinException("Seed is too short to support the required security level.");
	}

#ifdef acuteCOIN_DEBUG
	...
	*resultGroupOrder = generateRandomPrime(qLen, seed, &qseed, &qgen_counter);
	...
	// Throw an exception.
	throw acutecoinException("Unable to generate a prime modulus for the group");
}
...
/// \throws                             A acutecoinException if error.
///
...
calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, Bignum modulus, Bignum groupOrder, uint32_t index)
{
	...
	if (index > 255) {
		throw acutecoinException("Invalid index for group generation");
	}
	...
	throw acutecoinException("Unable to find a generator, too many attempts");
}
                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 2) {
		throw acutecoinException("Prime length is too short");
	}
	...
	if (primeBitLen < 33) {
		...
		throw acutecoinException("Unable to find prime in Shawe-Taylor algorithm");
	...
	throw acutecoinException("Unable to generate random prime (too many tests)");
}
