...
* @file       Tests.cpp
*
* @brief      Test routines for acutecoin.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", acuteCOIN_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),acuteCOIN_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	printf("acuteCoin v%s self-test routine\n", acuteCOIN_VERSION_STRING);
	...
	// Run through all of the acutecoin tests
